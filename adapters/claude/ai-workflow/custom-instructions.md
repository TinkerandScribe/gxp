...
2. **Phase 0.5 – Model Selection + Scaffolding tier**: Choose the right model before committing to a brief. Spend the least capability that clears the criteria with margin: **Opus 4.8** (or current flagship) for reasoning-heavy/Full-Workflow work (add Fast mode when latency matters), **Sonnet 4.6** for well-scoped implementation, **Haiku 4.5** for mechanical/bulk work; hand off to a sibling tool (Perplexity for research, the Grok/GXP loop for autonomous long-runs, a local model for offline/cheap work) when clearly better suited. **Also select a scaffolding tier** (`frontier` | `standard` | `constrained`) per `core/docs/capability-scaffolding.md` and the Claude model→tier map in `model-routing.md`: apply context-load policy after selection (minimal host scaffolding on frontier; full defaults on standard; denser steps/gates on constrained). Detection: explicit override → model map → default **standard**. Record **Model** and **Scaffolding tier** in the brief; re-evaluate both at Phase 4. Do not weaken binary criteria or verification for any tier; do not silently delete host CLAUDE.md/skills.
