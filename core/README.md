...
- All adapters (Cursor, Grok chat, Grok Build, Claude, ChatGPT, Codex, Perplexity, Cowork)
  derive from the content here.
- Changes to the process, templates, or principles should be made in `core/` first, then
  ported into the adapters (regenerate chat workflows with
  `python scripts/generate-adapter-workflows.py` when `workflow.md` changes).
...
- `workflow.md` — the canonical description of the full loop (phases, principles,
  anti-loop rule, verification order, optional ontology validation in Phase 5).
...
- `templates/` — reusable templates for task briefs, failure capture, weekly review, and
  optional domain ontology examples (`templates/ontology/`).
